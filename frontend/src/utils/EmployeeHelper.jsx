...
export const columns = [
  ...
  },
  {
    ...
    selector: (row) => row.image ? (
      <img
        src={`https://employee-a0gcfmzec-visheshsri20s-projects.vercel.app//uploads/${row.image}`}
        alt="Profile"
...
export const getEmployees = async (id) => {
  let employees;
  try {
    const response = await axios.get(`https://employee-a0gcfmzec-visheshsri20s-projects.vercel.app//api/employee/department/${id}`, {
      headers: {
        "Authorization": `Bearer ${localStorage.getItem('token')}`
      }
    });
