...
const EditDepartment = () => {
    ...
    useEffect(() => {
        const fetchDepartment = async () => {
            setDepLoading(true);
            try {
                const response = await axios.get(`https://employee-a0gcfmzec-visheshsri20s-projects.vercel.app//api/department/${id}`, {
                    headers: {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault()
        try {
            const response = await axios.put(`https://employee-a0gcfmzec-visheshsri20s-projects.vercel.app//api/department/${id}`, department,
                {
                    headers: {
                        "Authorization": `Bearer ${localStorage.getItem('token')}`
                    }
